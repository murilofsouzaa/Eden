...
INSERT INTO product (title, description, image_url, modeling, weight, material, created_at, updated_at) VALUES
	...
	('Black Compression Long Tee', 'Camiseta longa de compressão com recortes ergonômicos e toque frio.',
	...
	('Gym King Vest Black', 'Regata esportiva preta com recortes lineares e ajuste respirável.',
	...
	('Gym Tops & Tees Men', 'Coleção de tops e camisetas masculinas com foco em performance.',
	...
	('Sports & Fitness Releases', 'Imagem de lançamento com destaque para roupas esportivas e fitness.',
	...
	('Hyperflex Crop Tee Sky', 'Camiseta cropped azul sky com modelagem leve para treino.',
	...
	('Gymshark Crop Tank Navy', 'Top cropped seamless azul-marinho com acabamento sem costura.',
	...
	('Gymshark Crop Tank Black', 'Top cropped preto com caimento extra ajustado e visual moderno.',
	...
	('Vital Long Sleeve Top', 'Top de manga longa preto mescla com recortes e estilo atlético.',
...
INSERT INTO product (title, description, image_url, modeling, weight, material, created_at, updated_at) VALUES
	('Vuori Tank Navy KORE', 'Regata Vuori navy da coleção KORE com malha leve e respirável.',
	...
	('Vuori Tank Grey KORE', 'Regata Vuori sky grey da coleção KORE com caimento atlético.',
	...
	('Vuori Muscle Tee Chambray', 'Camiseta muscle Vuori chambray da coleção KORE com tecido stretch-knit.',
	...
	('Vuori Styles by Kaia', 'Seleção feminina KORE inspirada em looks de treino e lifestyle.',
	...
	('Vuori Legging Blue Coast', 'Legging AllTheFeels KORE Blue Coast com toque macio e compressão leve.',
...
INSERT INTO product_variant (product_id, sku, color, size, price, stock, category, gender, status, is_default) VALUES
	((SELECT id FROM product WHERE title = 'Gymshark Black Compression Tee'), 'SKU-0001', 'BLACK', 'M', 129.90, 35, 'T_SHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Gymshark Black Compression Tee'), 'SKU-0001-L', 'BLACK', 'L', 129.90, 18, 'T_SHIRTS', 'MASCULINE', 'AVAILABLE', FALSE),
	((SELECT id FROM product WHERE title = 'Gymshark Blue Oversized Hoodie'), 'SKU-0002', 'NAVY', 'M', 229.90, 18, 'SWEATSHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Gymshark Blue Oversized Hoodie'), 'SKU-0002-L', 'NAVY', 'L', 229.90, 10, 'SWEATSHIRTS', 'MASCULINE', 'AVAILABLE', FALSE),
	((SELECT id FROM product WHERE title = 'White Performance Joggers'), 'SKU-0003', 'WHITE', 'M', 179.90, 22, 'PANTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Lima Flex Training Shorts'), 'SKU-0004', 'LIME', 'M', 99.90, 40, 'SHORTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Black Compression Long Tee'), 'SKU-0005', 'BLACK', 'M', 119.90, 27, 'T_SHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Onyx Compression Long Tee'), 'SKU-0006', 'ONYX', 'M', 129.90, 18, 'T_SHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Carbon Compression Tee'), 'SKU-0007', 'CARBON', 'M', 129.90, 31, 'T_SHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Elite Compression Pro Top'), 'SKU-0008', 'BLACK', 'M', 139.90, 19, 'T_SHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Gymshark Black Oversized Tee'), 'SKU-0009', 'BLACK', 'M', 149.90, 25, 'SHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Pro Training Shorts'), 'SKU-0010', 'BLACK', 'M', 99.90, 34, 'SHORTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Gymshark Green Oversized Tee'), 'SKU-0011', 'GREEN', 'M', 149.90, 21, 'SHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	...
	((SELECT id FROM product WHERE title = 'Gym King Vest Black'), 'SKU-0027', 'BLACK', 'M', 99.90, 26, 'REGATTA', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Gymshark Shark Hoodie Black'), 'SKU-0028', 'BLACK', 'M', 249.90, 16, 'SWEATSHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Gymshark Shark Hoodie White'), 'SKU-0029', 'WHITE', 'M', 249.90, 14, 'SWEATSHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Vital Leggings Men'), 'SKU-0030', 'BLACK', 'M', 149.90, 20, 'LEGGING', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Gym Tops & Tees Men'), 'SKU-0031', 'BLACK', 'M', 119.90, 24, 'T_SHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Oxy Mass Muscle Black Tee'), 'SKU-0032', 'BLACK', 'M', 129.90, 29, 'T_SHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Sports & Fitness Releases'), 'SKU-0033', 'MULTI', 'U', 89.90, 12, 'SHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	...
	((SELECT id FROM product WHERE title = 'Hyperflex Crop Tee Sky'), 'SKU-0038', 'SKY_BLUE', 'S', 99.90, 33, 'SHIRTS', 'FEMININE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Hyperflex Shorts Gray'), 'SKU-0039', 'CHARCOAL', 'S', 89.90, 35, 'SHORTS', 'FEMININE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Gymshark Crop Tank Navy'), 'SKU-0040', 'NAVY', 'S', 109.90, 30, 'T_SHIRTS', 'FEMININE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Gymshark Crop Tank Black'), 'SKU-0041', 'BLACK', 'S', 109.90, 28, 'T_SHIRTS', 'FEMININE', 'AVAILABLE', TRUE),
	...
	((SELECT id FROM product WHERE title = 'Vital Long Sleeve Top'), 'SKU-0045', 'BLACK MARL', 'S', 129.90, 23, 'SHIRTS', 'FEMININE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Gymshark Sports Top 30-04-26'), 'SKU-0046', 'BLACK', 'S', 109.90, 34, 'T_SHIRTS', 'FEMININE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Vuori Viewpoint Muscle Tank Navy'), 'SKU-0047', 'NAVY', 'M', 149.90, 26, 'REGATTA', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Black White Workout Set'), 'SKU-0050', 'BLACK_WHITE', 'M', 199.90, 18, 'SET', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Vuori Tank Navy KORE'), 'SKU-0051', 'NAVY', 'M', 149.90, 24, 'REGATTA', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Vuori Tank Grey KORE'), 'SKU-0052', 'SKY_GREY', 'M', 149.90, 22, 'REGATTA', 'MASCULINE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Vuori Muscle Tee Chambray'), 'SKU-0053', 'CHAMBRAY', 'M', 139.90, 20, 'T_SHIRTS', 'MASCULINE', 'AVAILABLE', TRUE),
	...
	((SELECT id FROM product WHERE title = 'Vuori Styles by Kaia'), 'SKU-0056', 'MULTI', 'M', 239.90, 14, 'SET', 'FEMININE', 'AVAILABLE', TRUE),
	((SELECT id FROM product WHERE title = 'Vuori Legging Blue Coast'), 'SKU-0057', 'BLUE_COAST', 'M', 179.90, 17, 'LEGGING', 'FEMININE', 'AVAILABLE', TRUE),
...
INSERT INTO shopping_cart_item (cart_id, variant_id, quantity, unit_price) VALUES
	((SELECT id FROM shopping_cart WHERE user_id = (SELECT id FROM users WHERE email = 'lara.costa@eden.com')),
	 (SELECT pv.id FROM product_variant pv JOIN product p ON p.id = pv.product_id WHERE p.title = 'Gymshark Black Compression Tee' AND pv.is_default = TRUE), 2, 129.90),
	((SELECT id FROM shopping_cart WHERE user_id = (SELECT id FROM users WHERE email = 'lara.costa@eden.com')),
	...
	((SELECT id FROM shopping_cart WHERE user_id = (SELECT id FROM users WHERE email = 'diego.martins@eden.com')),
	 (SELECT pv.id FROM product_variant pv JOIN product p ON p.id = pv.product_id WHERE p.title = 'Gymshark Blue Oversized Hoodie' AND pv.is_default = TRUE), 1, 229.90),
	((SELECT id FROM shopping_cart WHERE user_id = (SELECT id FROM users WHERE email = 'diego.martins@eden.com')),
